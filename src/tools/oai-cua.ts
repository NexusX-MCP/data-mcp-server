...
export const oaiCuaToolDescription = 'General-purpose automation using OpenAI's CUA model';

export async function oaiCuaTool(params: {
  task: string;
  sessionOptions?: {
    useProxy?: boolean;
    useStealth?: boolean;
  ...
  try {
    ...
    const result = await client.browser.openaiCua(params.task, {
...
export async function oaiCuaToolOld({
  task,
  sessionOptions,
  returnStepInfo,
  maxSteps,
